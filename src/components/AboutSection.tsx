  ...
  return (
    <section id="about" className="py-20 md:py-28 section-alt">
      <div className="container" ref={ref}>
        {/* About Header */}
        <div className={`text-center max-w-3xl mx-auto ${isVisible ? "animate-reveal-up" : "opacity-0"}`}>
        ...
        {/* About Content */}
        <div className={`mt-8 max-w-3xl mx-auto space-y-5 text-muted-foreground leading-relaxed text-pretty ${isVisible ? "animate-reveal-up delay-200" : "opacity-0"}`}>
          <p>
        ...
        {/* Stats */}
        <div className={`mt-12 grid grid-cols-2 md:grid-cols-4 gap-8 ${isVisible ? "animate-reveal-up delay-300" : "opacity-0"}`}>
          {stats.map((s) => (
        ...
        {/* Divider */}
        <div className={`mt-16 mb-14 h-px bg-border ${isVisible ? "animate-reveal-up delay-400" : "opacity-0"}`} />

        {/* Team Header */}
        <div className={`text-center max-w-2xl mx-auto ${isVisible ? "animate-reveal-up delay-400" : "opacity-0"}`}>
        ...
        {/* Team Cards */}
        <div className={`mt-10 flex justify-center ${isVisible ? "animate-reveal-up delay-500" : "opacity-0"}`}>
          {team.map((member) => (
            <div
              key={member.name}
              className="group max-w-xs w-full bg-card rounded-2xl border border-border/50 shadow-lg shadow-primary/5 overflow-hidden transition-shadow hover:shadow-xl hover:shadow-primary/10"
            >
